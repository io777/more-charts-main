...
deploy:
	...
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER} && echo "COMPOSE_PROJECT_NAME=more-charts" >> .env'
	...
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'mkdir -p site_${BUILD_NUMBER}/production/secrets'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${SQL_USER}" >> sql_user'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${SQL_PORT}" >> sql_port'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${SQL_PASSWORD}" >> sql_password'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${SQL_HOST}" >> sql_host'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${SQL_ENGINE}" >> sql_engine'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${SQL_DATABASE}" >> sql_database'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${EMAIL_USE_SSL}" >> email_use_ssl'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${EMAIL_PORT}" >> email_port'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${EMAIL_HOST_USER}" >> email_host_user'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${EMAIL_HOST}" >> email_host'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${DJANGO_SECRET_KEY}" >> django_secret_key'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${DJANGO_PUBG_API3}" >> django_pubg_api3'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${DJANGO_PUBG_API2}" >> django_pubg_api2'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${DJANGO_PUBG_API1}" >> django_pubg_api1'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${DJANGO_DEBUG}" >> django_debug'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER}/production/secrets && echo "${DJANGO_ALLOWED_HOSTS}" >> django_allowed_hosts'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'chmod -R 777 site_${BUILD_NUMBER}'
