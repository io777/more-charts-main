...
deploy:
	...
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'mkdir /root/site_${BUILD_NUMBER}/production/secrets'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${SQL_USER}" >> /root/site_${BUILD_NUMBER}/production/secrets/sql_user'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${SQL_PORT}" >> /root/site_${BUILD_NUMBER}/production/secrets/sql_port'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${SQL_PASSWORD}" >> /root/site_${BUILD_NUMBER}/production/secrets/sql_password'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${SQL_HOST}" >> /root/site_${BUILD_NUMBER}/production/secrets/sql_host'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${SQL_ENGINE}" >> /root/site_${BUILD_NUMBER}/production/secrets/sql_engine'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${SQL_DATABASE}" >> /root/site_${BUILD_NUMBER}/production/secrets/sql_database'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${EMAIL_USE_SSL}" >> /root/site_${BUILD_NUMBER}/production/secrets/email_use_ssl'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${EMAIL_PORT}" >> /root/site_${BUILD_NUMBER}/production/secrets/email_port'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${EMAIL_HOST_USER}" >> /root/site_${BUILD_NUMBER}/production/secrets/email_host_user'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${EMAIL_HOST}" >> /root/site_${BUILD_NUMBER}/production/secrets/email_host'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${DJANGO_SECRET_KEY}" >> /root/site_${BUILD_NUMBER}/production/secrets/django_secret_key'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${DJANGO_PUBG_API3}" >> /root/site_${BUILD_NUMBER}/production/secrets/django_pubg_api3'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${DJANGO_PUBG_API2}" >> /root/site_${BUILD_NUMBER}/production/secrets/django_pubg_api2'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${DJANGO_PUBG_API1}" >> /root/site_${BUILD_NUMBER}/production/secrets/django_pubg_api1'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${DJANGO_DEBUG}" >> /root/site_${BUILD_NUMBER}/production/secrets/django_debug'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${DJANGO_ALLOWED_HOSTS}" >> /root/site_${BUILD_NUMBER}/production/secrets/django_allowed_hosts'
