...
deploy:
	...
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${SQL_USER}" >> site_${BUILD_NUMBER}/production/secrets/sql_user'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${SQL_PORT}" >> site_${BUILD_NUMBER}/production/secrets/sql_port'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${SQL_PASSWORD}" >> site_${BUILD_NUMBER}/production/secrets/sql_password'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${SQL_HOST}" >> site_${BUILD_NUMBER}/production/secrets/sql_host'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${SQL_ENGINE}" >> site_${BUILD_NUMBER}/production/secrets/sql_engine'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${SQL_DATABASE}" >> site_${BUILD_NUMBER}/production/secrets/sql_database'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${EMAIL_USE_SSL}" >> site_${BUILD_NUMBER}/production/secrets/email_use_ssl'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${EMAIL_PORT}" >> site_${BUILD_NUMBER}/production/secrets/email_port'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${EMAIL_HOST_USER}" >> site_${BUILD_NUMBER}/production/secrets/email_host_user'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${EMAIL_HOST}" >> site_${BUILD_NUMBER}/production/secrets/email_host'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${DJANGO_SECRET_KEY}" >> site_${BUILD_NUMBER}/production/secrets/django_secret_key'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${DJANGO_PUBG_API3}" >> site_${BUILD_NUMBER}/production/secrets/django_pubg_api3'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${DJANGO_PUBG_API2}" >> site_${BUILD_NUMBER}/production/secrets/django_pubg_api2'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${DJANGO_PUBG_API1}" >> site_${BUILD_NUMBER}/production/secrets/django_pubg_api1'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${DJANGO_DEBUG}" >> site_${BUILD_NUMBER}/production/secrets/django_debug'
	ssh -o StrictHostKeyChecking=no root@${HOST} -p ${PORT} 'echo "${DJANGO_ALLOWED_HOSTS}" >> site_${BUILD_NUMBER}/production/secrets/django_allowed_hosts'
